...
/** Ecwid-generated storefront options (Copán Rise product view). */
...
/**
 * Ecwid install code for Copán Rise: product browser + storefront layout.
 * Uses the global script from EcwidProvider (same store as script.js?133781758&data_platform=code).
 * @param {'home' | 'origin'} placement — unique widget container id per page
 */
export default function EcwidCopanRiseEmbed({ placement = 'home' }) {
  const { ready } = useEcwid();
  const ran = useRef(false);
  const productId = ECWID_PRODUCT_BY_ORIGIN.honduras;
  ...
  return (
    <div
      className={
        isHome
          ? 'w-full min-h-[100px] max-h-[min(70vh,520px)] overflow-y-auto overflow-x-hidden [&_.ec-store]:text-left'
          : 'w-full min-h-[120px] [&_.ec-store]:text-left'
      }
    >
